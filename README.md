...
>
> 被跳过的 claude mcp 注册如果是别人的版本而不是团队版（命令行参数不同），跑 `npx -y foodmax-ai-config@latest update --force-mcp` 强制重装。
...
```bash
npx -y foodmax-ai-config@latest update                # 拉 latest dist-tag
npx -y foodmax-ai-config@latest update --version 1.0.0  # pin 到具体版本
npx -y foodmax-ai-config@latest update --tag beta     # 切到 beta dist-tag
npx -y foodmax-ai-config@latest update --force-mcp    # 维护者通知 MCP 注册参数变了时用
```
...
```bash
npx -y foodmax-ai-config@latest status              # 列 modified / added / removed
npx -y foodmax-ai-config@latest status --diff       # 同上，每个 modified 出一段 diff
```
...
```bash
npx -y foodmax-ai-config@latest verify              # 有 drift 也 exit 0
npx -y foodmax-ai-config@latest verify --strict     # 有 drift exit 1（init 写出来的 CI workflow 就用这条）
```

### 一键修复

```bash
npx -y foodmax-ai-config@latest repair
```
...
- 想丢 → `npx -y foodmax-ai-config@latest repair`
...
| 机制 | 来源 | 怎么用 |
|---|---|---|
| **npm dist-tags**（channels）| npm registry 原生 | `npx -y foodmax-ai-config@latest update --tag latest` / `--tag beta` |
| **npm deprecate**（warn）| `npm deprecate foodmax-ai-config@X.Y.Z "<reason>"` | 同事跑 `npm install` 时 npm 自动打 warning |
| **`.locked.json` sha256**（drift detection）| 包内构建产物 | `npx -y foodmax-ai-config@latest verify` 检查 |
| **`.foodmax-ai.lock.json`**（项目 pin）| init/update 写 | `npx -y foodmax-ai-config@latest repair` 按这里的 packageVersion 重装 |
...
| `verify --strict` 在 CI exit 1 | 本地跑 `npx -y foodmax-ai-config@latest status --diff` 看 drift；恢复用 `repair` |
...
> ⚠️ 本次升级修改了 MCP 注册参数，请用 `npx -y foodmax-ai-config@latest update --force-mcp` 升级。
